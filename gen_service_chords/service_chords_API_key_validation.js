...
app.get('/api/keys', async (req, res) => {
  try {
    const result = await callJavaJar(['keys']);
    res.json(result);
  } catch (err) {
...
app.get('/api/structures', async (req, res) => {
  try {
    const result = await callJavaJar(['structures']);
    res.json(result);
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: 'Failed to fetch structures', details: err.message });
  }
});

// GET /api/modulations
app.get('/api/modulations', async (req, res) => {
  try {
    const result = await callJavaJar(['modulations']);
    res.json(result);
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: 'Failed to fetch modulations', details: err.message });
  }
});
...
app.get('/api/generate/:key/:structure/:modulation', async (req, res) => {
  ...
  try {
    ...
    // Log using values returned by the JAR
    logToCsv({
      result,
      timeMs,
      sizeBytes
